...
namespace ROIGenerateLib
{
    ...
    public class ROICircularArc : ROI
	{
        ...
        public ROICircularArc(int imagewidth, int imageheight) : base(imagewidth, imageheight)
        {
            ...
            ROIConstanceSymbolInfo = EumROIConstanceSymbolInfo.CircularArc;
            ActiveBox = (int)(imageheight * Scale_factor);
        }
        ...
        {
            return string.Format("ROICircularArc_{0}", currTitleIndex);
		...
		public override void draw(HalconDotNet.HWindow window)
		{
			...
			window.DispRectangle2(sizeR, sizeC, 0, ActiveBox, ActiveBox);
			window.DispRectangle2(midR, midC, 0, ActiveBox, ActiveBox);
			window.DispRectangle2(startR, startC, startPhi, ActiveBox, ActiveBox/2);
		...
		public override HRegion getRegion()
		{
            ...
            region.Dispose();
            region.GenCircleSector(midR, midC, radius, startPhi, (startPhi + extentPhi));
		...
		private void updateArrowHandle()
		{
			...
			row1 = row2 + sign * Math.Sin(angleRad) * 30;
			col1 = col2 - sign * Math.Cos(angleRad) * 30;
