...
namespace ROIGenerateLib
{
	...
	public class ROIRectangle2 : ROI
	{
        ...
        public ROIRectangle2(int imagewidth, int imageheight) : base(imagewidth, imageheight)
        {
            currTitleIndex= ++titleIndex;
            NumHandles = 6; // 4 corners +  1 midpoint + 1 rotationpoint			
			activeHandleIdx = 4;
            ROIConstanceSymbolInfo = EumROIConstanceSymbolInfo.Rectangle2;
        }
        ...
        {
            return string.Format("ROIRectangle2_{0}", currTitleIndex);
		...
		public override void draw(HalconDotNet.HWindow window)
		{
			...
			for (int i =0; i < NumHandles; i++)
				window.DispRectangle2(rows[i].D, cols[i].D,phi, ActiveBox, ActiveBox);

			window.DispArrow(midR, midC, midR + (Math.Sin(-phi) * length1 * 1.2),
				midC + (Math.Cos(-phi) * length1 * 1.2), ActiveBox/2);
