...
namespace PositionToolsLib.工具
{
    ...
    public  class MatchTool: BaseTool,IDisposable
    {
        ...
        public bool TemplateTraining(out HObject modelTrans)
        {
            ...
            /*----加入图像格式判断防止多通道----*/
            #region---图像格式判断---
            HOperatorSet.CountChannels((toolParam as MatchParam).InputImg, out HTuple channels);
            HObject grayImage = null;
            if (channels[0].I > 1)
                HOperatorSet.Rgb1ToGray((toolParam as MatchParam).InputImg, out grayImage);
            else
                grayImage = (toolParam as MatchParam).InputImg.Clone();
            #endregion

            HOperatorSet.ReduceDomain(grayImage, subROI, out HObject ho_ImageReduced);
            ...
            ho_ImageReduced2.Dispose();
            HOperatorSet.ReduceDomain(grayImage, modelRegion,
        ...
        override public RunResult Run()
        {
            ...
            try
            {
                ...
                //输入检测区域无效则为全图检测
                    ...
                    HOperatorSet.CopyObj(grayImage, out imageReduced, 1, 1);
                ...
                string info = string.Format("模板匹配OK,坐标x:{0:f3},y:{1:f3},r:{2:f3}", hv_Column.D, hv_Row.D, hv_Angle.TupleDeg().D) ;
                if (!dm.resultInfoDic.ContainsKey(toolName))
