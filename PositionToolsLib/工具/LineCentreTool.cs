...
namespace PositionToolsLib.工具
{
    ...
    /// </summary>
    [Serializable]
    public class LineCentreTool : BaseTool, IDisposable
    {
        public static int inum = 0;//工具编号

        public LineCentreTool()
        {
            toolParam = new LineCentreParam();
            toolName = "直线中心" + inum;
            inum++;
        }
        ...
        //工具日志:同类型工具日志信息放一起      
        static private Log log = new Log("直线中心");

        [OnDeserialized()]
        internal void OnDeserializedMethod(StreamingContext context)
        {

            int number = int.Parse(toolName.Replace("直线中心", ""));
            if (number > inum)
                inum = number;
            //toolName = "直线中心" + number;
            inum++;
        }
        /// <summary>
        /// 直线中心工具运行
        ...
        override public RunResult Run()
        {
            ...
            { 
                ...
                {
                    (toolParam as LineCentreParam).LineCentreRunStatus = false;
                ...
                double x1 = 0, y1= 0;
                if (dm.resultFlagDic[(toolParam as LineCentreParam).StartXName.Replace("起点","").Replace("终点", "")])
                {
                    StuCoordinateData xDat = dm.PositionDataDic[(toolParam as LineCentreParam).StartXName];
                    x1 = xDat.x;                   
                }
                if (dm.resultFlagDic[(toolParam as LineCentreParam).StartYName.Replace("起点", "").Replace("终点", "")])
                {
                    StuCoordinateData yDat = dm.PositionDataDic[(toolParam as LineCentreParam).StartYName];
                    y1 = yDat.y;
                }
                double x2 = 0, y2 = 0;
                if (dm.resultFlagDic[(toolParam as LineCentreParam).EndXName.Replace("起点", "").Replace("终点", "")])
                {
                    StuCoordinateData xDat = dm.PositionDataDic[(toolParam as LineCentreParam).EndXName];
                    x2 = xDat.x;
                }
                if (dm.resultFlagDic[(toolParam as LineCentreParam).EndYName.Replace("起点", "").Replace("终点", "")])
                {
                ...
                if (!dm.resultBufDic.ContainsKey(toolName))
                    ...
                    dm.resultBufDic[toolName] = cross.Clone();
