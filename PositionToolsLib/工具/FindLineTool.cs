...
namespace PositionToolsLib.工具
{
    ...
    /// </summary>
    [Serializable]
    public class FindLineTool : BaseTool, IDisposable
    {
        public static int inum = 0;//工具编号

        public FindLineTool()
        {
            toolParam = new FindLineParam();
            toolName = "查找直线" + inum;
            inum++;
        }
        ...
        //工具日志:同类型工具日志信息放一起      
        static private Log log = new Log("查找直线");
        ...
        internal void OnDeserializedMethod(StreamingContext context)
        {
            ...
            int number = int.Parse(toolName.Replace("查找直线", ""));
            if (number > inum)
                inum = number;
            //toolName = "查找直线" + number;
            inum++;
        }
        /// <summary>
        /// 查找直线工具运行
        ...
        override public RunResult Run()
        {
            ...
            try
            {
                ...
                {
                    (toolParam as FindLineParam).FindLineRunStatus = false;
                ...
                /*----加入图像格式判断防止多通道----*/
                #region---图像格式判断---
                HOperatorSet.CountChannels((toolParam as FindLineParam).InputImg, out HTuple channels);
                HObject grayImage = null;
                if (channels[0].I > 1)
                    HOperatorSet.Rgb1ToGray((toolParam as FindLineParam).InputImg, out grayImage);
                else
                    grayImage = (toolParam as FindLineParam).InputImg.Clone();
                #endregion
                if (!ObjectValided((toolParam as FindLineParam).InspectXLD))
                {
                    (toolParam as FindLineParam).FindLineRunStatus = false;
                    result.runFlag = false;
                ...
                //仿射变换矩阵
                HObject xld = (toolParam as FindLineParam).InspectXLD;
                if ((toolParam as FindLineParam).UsePosiCorrect)
                {
                    HTuple matrix2D = dm.matrixBufDic[(toolParam as FindLineParam).MatrixName];
                    if (matrix2D != null)
                        HOperatorSet.AffineTransContourXld((toolParam as FindLineParam).InspectXLD,
                          out xld, matrix2D);
                    else
                    {
                        if (!dm.resultFlagDic.ContainsKey(toolName))
                ...
                get_rake_region(grayImage, out HObject ho_Regions,
                   (toolParam as FindLineParam).CaliperNum, (toolParam as FindLineParam).CaliperHeight,
                    (toolParam as FindLineParam).CaliperWidth, hv_Row1, hv_Column1, hv_Row2, hv_Column2);
                (toolParam as FindLineParam).ResultInspectROI = ho_Regions.Clone();
                //卡尺
                rake(grayImage, out HObject ho_Regions2,
                    (toolParam as FindLineParam).CaliperNum, (toolParam as FindLineParam).CaliperHeight,
                    (toolParam as FindLineParam).CaliperWidth, 1.0,
                    (toolParam as FindLineParam).EdgeThd,
                    Enum.GetName(typeof(EumTransition), (toolParam as FindLineParam).Transition),
                    Enum.GetName(typeof(EumSelect), (toolParam as FindLineParam).Select),
                    hv_Row1, hv_Column1,
                    hv_Row2, hv_Column2,
                ...
                if (hv_ResultRow.TupleLength() <= 1)
                {
                    ...
                    (toolParam as FindLineParam).FindLineRunStatus = false;
                    result.runFlag = false;
                    result.errInfo = toolName + "直线查找失败";
                    return result;
                }
                ...
                //测试结果
                if (!dm.resultFlagDic.ContainsKey(toolName))
                    dm.resultFlagDic.Add(toolName, hv_Row11.TupleLength() > 0);
                else
                    dm.resultFlagDic[toolName] = hv_Row11.TupleLength() > 0;

                grayImage.Dispose();
                //+输入图像

                HOperatorSet.ConcatObj((toolParam as FindLineParam).InputImg, emptyRegionBuf, out HObject objectsConcat2);
                (toolParam as FindLineParam).OutputImg = objectsConcat2;
                (toolParam as FindLineParam).StartPointRow = hv_Row11.D;
                (toolParam as FindLineParam).StartPointColumn = hv_Column11.D;
                ...
                if (!dm.LineDataDic.ContainsKey(toolName))
                    dm.LineDataDic.Add(toolName, new StuLineData(hv_Row11.D, hv_Column11.D,
                        hv_Row21.D, hv_Column21.D));
                else
                    dm.LineDataDic[toolName] = new StuLineData(hv_Row11.D, hv_Column11.D,
                        hv_Row21.D, hv_Column21.D);

                result.runFlag = true;
                (toolParam as FindLineParam).FindLineRunStatus = true;
