...
namespace PositionToolsLib.窗体.ViewModels
{
    internal class CalParallelLineViewModel : BaseViewModel
    {
        ...
        public CalParallelLineViewModel(BaseTool tool) : base(tool)
        {
            dataManage = tool.GetManage();
            This = this;
            Model = new CalParallelLineModel();
            //图像控件      
        ...
        private void cobxImageList_SelectedIndexChanged(object value)
        {
            ...
            BaseParam par = baseTool.GetParam();
            (par as CalParallelLineParam).InputImageName = Model.SelectImageName;
        }
        ...
        {
            BaseParam par = baseTool.GetParam();
            (par as CalParallelLineParam).LineData = lineData1;
            (par as CalParallelLineParam).LineData2 = lineData2;
            (par as CalParallelLineParam).InputLineName = Model.SelectLine1Name;
            (par as CalParallelLineParam).InputLine2Name = Model.SelectLine2Name;
        ...
        private void btnTest_Click()
        {
            ...
            if (rlt.runFlag)
            {
                ...
                ShowTool.DispConcatedObj((par as CalParallelLineParam).OutputImg, EumCommonColors.green);
                ShowTool.AddConcatedObjBuffer((par as CalParallelLineParam).OutputImg, EumCommonColors.green);

                ShowTool.DispMessage("OK", 10, width - 500, "green", 100);
                ShowTool.AddTextBuffer("OK", 10, width - 500, "green", 100);
                //更新结果表格数据
                UpdateResultView(new ParallelLineData(1,
                    (par as CalParallelLineParam).ParallelLineColumn1,
                    (par as CalParallelLineParam).ParallelLineRow1,
                     (par as CalParallelLineParam).ParallelLineColumn2,
                     (par as CalParallelLineParam).ParallelLineRow2,                     
                      (par as CalParallelLineParam).ParallelLineAngle
                 ));
            }
            else
            {
        ...
        {
            Model.ParallelLineDataList.Clear();
            Model.ParallelLineDataList.Add(Data);
