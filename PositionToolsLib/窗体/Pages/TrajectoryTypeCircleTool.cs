...
{
    [Serializable]
    public class TrajectoryTypeCircleTool : TrajectoryTypeBaseTool
    {
        public TrajectoryTypeCircleTool()
        {
            base.param = new TrajectoryTypeCircleParam();
        }
        ...
        public override TemRunResult Run()
        {
            ...
            try
            {
                ...
                if (!ObjectValided(((TrajectoryTypeCircleParam)param).InspectXLD))
                {
                    result.runFlag = false;
                ...
                HObject xld = (param as TrajectoryTypeCircleParam).InspectXLD;
                HOperatorSet.GetContourXld(xld, out HTuple hv_Rows, out HTuple hv_Columns);
                get_spoke_region(param.InputImage, out HObject ho_Regions,
                  (param as TrajectoryTypeCircleParam).CaliperNum, (param as TrajectoryTypeCircleParam).CaliperHeight,
                   (param as TrajectoryTypeCircleParam).CaliperWidth, hv_Rows, hv_Columns,
                     Enum.GetName(typeof(EumDirection), (param as TrajectoryTypeCircleParam).Direction));
               
                param.ResultInspectROI = ho_Regions.Clone();
                //卡尺
                spoke(param.InputImage, out HObject ho_Regions2,
                   (param as TrajectoryTypeCircleParam).CaliperNum, (param as TrajectoryTypeCircleParam).CaliperHeight,
                   (param as TrajectoryTypeCircleParam).CaliperWidth, 1.0,
                   (param as TrajectoryTypeCircleParam).EdgeContrast,
                   Enum.GetName(typeof(EumTransition), (param as TrajectoryTypeCircleParam).Transition),
                   Enum.GetName(typeof(EumSelect), (param as TrajectoryTypeCircleParam).Select),
                  hv_Rows, hv_Columns,
                     Enum.GetName(typeof(EumDirection), (param as TrajectoryTypeCircleParam).Direction),
                ...
                HOperatorSet.GenCrossContourXld(out HObject cross, hv_RowCenter.D, hv_ColCenter.D, 50, 0);
                HOperatorSet.ConcatObj(ho_Circle, cross, out HObject emptyRegionBuf);
          
                result.resultContour = emptyRegionBuf.Clone();
                result.trajectoryDataPoints.Add(new DgTrajectoryData(1, hv_ColCenter.D, hv_RowCenter.D, hv_Radius.D));
