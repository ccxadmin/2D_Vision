...
{
    [Serializable]
    public class TrajectoryTypeRRectangleTool : TrajectoryTypeBaseTool
    {
        public TrajectoryTypeRRectangleTool()
        {
            base.param = new TrajectoryTypeRRectangleParam();
        }
        public override TemRunResult Run()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    if (!ObjectValided(((TrajectoryTypeRRectangleParam)param).GetParamFormIndex(i).InspectXLD))
                    {
                        result.runFlag = false;
                        result.info = string.Format("输入检测区域{0}无效", "RRectangle:直线" + (i + 1));
                        return result;
                    }


                    HObject xld = (param as TrajectoryTypeRRectangleParam).GetParamFormIndex(i).InspectXLD;
                    ...
                    get_rake_region(param.InputImage, out HObject Region,
                      (param as TrajectoryTypeRRectangleParam).GetParamFormIndex(i).CaliperNum,
                      (param as TrajectoryTypeRRectangleParam).GetParamFormIndex(i).CaliperHeight,
                        (param as TrajectoryTypeRRectangleParam).GetParamFormIndex(i).CaliperWidth,
                    ...
                    rake(param.InputImage, out HObject ho_Regions2,
                        ...
                        (param as TrajectoryTypeRRectangleParam).GetParamFormIndex(i).CaliperWidth, 
                        1.0,
                       (param as TrajectoryTypeRRectangleParam).GetParamFormIndex(i).EdgeContrast,
                        Enum.GetName(typeof(EumTransition),
                       (param as TrajectoryTypeRRectangleParam).GetParamFormIndex(i).Transition),
                        Enum.GetName(typeof(EumSelect),
                        (param as TrajectoryTypeRRectangleParam).GetParamFormIndex(i).Select),
                        hv_Row1, hv_Column1,
                        hv_Row2, hv_Column2,
                ...
                HOperatorSet.SmallestRectangle2Xld(cocntour,out HTuple rowx,out HTuple columnx,out HTuple phix,
                    out HTuple length1x,out HTuple length2x);
                cocntour.Dispose();
                HOperatorSet.GenRectangle2ContourXld(out HObject rectanggle2, rowx,
                    columnx, phix, length1x, length2x);
                result.trajectoryDataPoints.RemoveRange(4,4);
                // *提取仿射最小外接矩形的四个顶点坐标
                HOperatorSet.TupleCos(phix, out HTuple Cos);
                HOperatorSet.TupleSin(phix, out HTuple Sin);
                HTuple a = -length1x * Cos - length2x * Sin;
                HTuple b = -length1x * Sin + length2x * Cos;
                HTuple pr1 = rowx - b;
                HTuple pc1 = columnx + a;
                ...
                HTuple c = length1x * Cos - length2x * Sin;
                HTuple d = length1x * Sin + length2x * Cos;
                HTuple pr2 = rowx - d;
                HTuple pc2 = columnx + c;
                ...
                HTuple e = length1x * Cos + length2x * Sin;
                HTuple f = length1x * Sin - length2x * Cos;
                HTuple pr3 = rowx - f;
                HTuple pc3 = columnx + e;
                ...
                HTuple g = -length1x * Cos + length2x * Sin;
                HTuple h = -length1x * Sin - length2x * Cos;
                HTuple pr4 = rowx - h;
                HTuple pc4 = columnx + g;
                ...
                HOperatorSet.ConcatObj(objectsConcat, rectanggle2, out objectsConcat);
                ...
                cross3.Dispose();
                cross4.Dispose();
                result.info = "旋转矩形轨迹检测完成";
                result.runFlag = true;                
            }
            catch (Exception er)
            {
                result.info = "旋转矩形轨迹检测异常:" + er.Message;
                result.runFlag = false;
                return result;
            }
            return result;
